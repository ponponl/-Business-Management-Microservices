...
import { approveContract, rejectContract, requestContractRevision } from '../../services/contractApi';
...
export default function ContractProcessingModal({ contract, customerName, role = 'MANAGER', onClose, onSuccess }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setError(submitError.message || 'Không thể xử lý hợp đồng.');
        } finally {
    ...
    return <div className="processing-modal-backdrop" role="presentation">
        <div className="processing-modal" role="dialog" aria-modal="true" aria-labelledby="processing-modal-title">
            ...
            <p className="processing-question">Bạn muốn thực hiện hành động gì với hợp đồng này?</p>
            <div className="processing-actions">{Object.entries(ACTIONS).map(([key, item]) => { const Icon = item.icon; const description = key === 'approve' && role === 'MANAGER' ? 'Chuyển hợp đồng sang bước Director review' : item.description; return <button key={key} type="button" className={`processing-action ${item.className} ${selectedAction === key ? 'selected' : ''}`} onClick={() => { setSelectedAction(key); setError(''); }} disabled={isSubmitting}><span className="processing-action-icon"><Icon size={19} /></span><span><strong>{item.title}</strong><small>{description}</small></span><Check size={17} className="processing-selected-check" /></button>; })}</div>
            <label className="processing-comment-label" htmlFor="processing-comment">Nhận xét {isCommentRequired ? '(bắt buộc nếu chọn yêu cầu chỉnh sửa hoặc từ chối)' : '(không bắt buộc)'}</label>
            <textarea id="processing-comment" value={comment} onChange={(event) => setComment(event.target.value)} maxLength={2000} placeholder="Nhập nhận xét của bạn..." disabled={isSubmitting} />
            <div className="processing-comment-count">{comment.length}/2000 ký tự</div>
            {error && <div className="processing-error" role="alert">{error}</div>}
            <div className="processing-modal-footer"><button className="processing-cancel" type="button" onClick={onClose} disabled={isSubmitting}>Hủy</button><button className="processing-confirm" type="button" onClick={handleSubmit} disabled={isSubmitting}>{isSubmitting ? 'Đang xử lý...' : 'Xác nhận'}</button></div>
        </div>
    </div>;
}
