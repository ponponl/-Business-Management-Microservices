...
export default function DirectorPriceListApprovalPage({ user }) {
  // Stats Card State
  const [stats, setStats] = useState({ approved: 0, effective: 0, rejected: 0 });
  ...
  const handleDirectorApprove = async (item) => {
    ...
    try {
      ...
      setModalConfig({
        ...
        message: `Bảng giá ${priceCode} đã có hiệu lực (EFFECTIVE).`
      });
    } catch (err) {
      ...
      <div className="bg-white p-2 rounded-lg border border-slate-200 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="bg-slate-100 p-0.5 rounded-md flex items-center space-x-0.5 text-[11px] font-medium text-slate-500 overflow-x-auto">
          {[
            ...
            <button 
              ...
              className={`px-2.5 py-0.5 rounded transition whitespace-nowrap cursor-pointer ${statusFilter === tab.key ? 'bg-white text-slate-800 shadow-xs font-semibold' : 'hover:text-slate-800'}`}
            >
              {tab.label}
            </button>
          ))}
        ...
        <div className="relative flex-1 sm:flex-initial min-w-[140px]">
          ...
          <input 
            ...
            className="w-full pl-7 pr-2 py-1 rounded border border-slate-200 text-xs focus:outline-none focus:border-amber-500 bg-white placeholder:text-slate-400" 
          />
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50 text-[11px] font-semibold text-slate-500">
                ...
                <th className="py-2.5 px-3">Người duyệt trước</th>
                ...
                priceLists.map((item) => {
                  ...
                  return (
                    <tr key={code} className="hover:bg-slate-50/80 transition border-b border-slate-100">
                      ...
                      <td className="py-2.5 px-3">
                        <div className="font-semibold text-slate-800 leading-snug">{approvedBy}</div>
