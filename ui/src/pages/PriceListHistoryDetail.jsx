...
export const PriceListHistoryDetail = ({
  ...
  useEffect(() => {
    ...
    const fetchContracts = async () => {
      try {
        ...
        const map = {};
        list.forEach((item) => {
          const id = item.contract_id || item.id;
          const code = item.contract_number || item.code || "";
          const name = item.contract_name || item.title || item.customer_name || "";
          if (id) map[String(id)] = { code, name };
          if (code) map[String(code)] = { code, name };
        });
        setContractsMap(map);
  ...
  const renderScopeLabelAndValue = (detail) => {
    ...
    const type = (detail.scope_type || detail.target_type || selectedVersion?.target_type || selectedVersion?.scope_type || "").toUpperCase();
    let label = "Đối tượng áp dụng cụ thể *";
    let value = detail.scope_name || detail.scope_id || detail.contract_id || detail.specific_target || detail.specificTarget || "";

    if (type === "GENERAL") {
      label = "Đối tượng áp dụng cụ thể *";
      value = "Áp dụng cho tất cả (Chung)";
    } else if (type === "CUSTOMER" || detail.customer_code || detail.customer_name) {
      label = "Khách hàng áp dụng *";
      const key = String(value || detail.customer_code || detail.customer_id || detail.customerId || detail.scope_id || "");
      const found = customersMap[key];
      if (found) {
        value = found.name ? `${found.name} - ${found.code || key}` : (found.code || key);
      } else if (detail.customer_name) {
        value = detail.customer_code ? `${detail.customer_name} - ${detail.customer_code}` : detail.customer_name;
      }
    } else if (type === "CONTRACT" || detail.contract_code || detail.contract_name || detail.contract_id) {
      label = "Số hợp đồng áp dụng *";
      const key = String(value || detail.contract_code || detail.contract_id || detail.contractId || detail.scope_id || "");
      const found = contractsMap[key];
      if (found) {
        value = found.code ? `${found.code} - ${found.name}` : found.name;
      } else {
